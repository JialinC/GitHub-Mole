...
const Login: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSSOConfig = async () => {
      try {
        console.log("Fetching SSO configuration...");
        const response = await axios.get("/api/helper/sso", { timeout: 5000 });
        console.log("SSO configuration fetched:", response.data);
  ...
  useEffect(() => {
    if (ssoEnabled === true) {
      ...
      window.location.href = "http://127.0.0.1:5000/oauth/authorize";
    }
  }, [ssoEnabled]);
        ...
        <main className="flex-grow container mx-auto p-4 flex flex-col items-center justify-center text-center">
          ...
          <div className="mt-8 bg-stone-900 bg-opacity-30 p-6 rounded-lg shadow-lg text-left">
            <p className="text-white text-sm">
              ...
              on github.com. Enterprise GitHub accounts cater to organizations
              and offer additional features and controls, including access to a
              private GitHub instance. A Personal Access Token (PAT) generated
              from a personal account can only be used to access the GitHub API
              at{" "}
