...
class TestUtilityFunctions:
    ...
    def test_add_by_days(self):
        ...
        expected_time = (
            datetime.strptime(original_time, "%Y-%m-%dT%H:%M:%SZ") + timedelta(days=365)
        ).strftime("%Y-%m-%dT%H:%M:%SZ")
        assert (
            add_by_days(original_time, 365) == expected_time
        ), "Should add one year to the input time string."
    ...
    def test_minus_by_days(self):
        ...
        assert (
            minus_by_days(original_time, 365) == expected_time
        ), "Should minus one year to the input time string."

    def test_in_time_period(self):
        start = "2021-01-01T00:00:00Z"
        end = "2021-12-31T23:59:59Z"
        assert (
            in_time_period("2021-06-01T12:00:00Z", start, end) is True
        ), "Should be within the time period."
        assert (
            in_time_period("2020-01-01T00:00:00Z", start, end) is False
        ), "Should be outside the time period."

    def test_created_before(self):
        assert (
            created_before("2021-01-01T00:00:00Z", "2022-01-01T00:00:00Z") is True
        ), "Created should be before the time."
        assert (
            created_before("2023-01-01T00:00:00Z", "2022-01-01T00:00:00Z") is False
        ), "Created should not be before the time."

    def test_created_after(self):
        assert (
            created_after("2022-01-01T00:00:00Z", "2021-01-01T00:00:00Z") is True
        ), "Created should be after the time."
        assert (
            created_after("2022-01-01T00:00:00Z", "2023-01-01T00:00:00Z") is False
        ), "Created should not be after the time."
    ...
    def test_write_csv(self):
        with tempfile.NamedTemporaryFile("w+", delete=False) as tmp:
            ...
            content = tmp.read()
            assert (
                content.strip() == data_row
            ), "The written content should match the input data row."
    ...
    def test_get_owner_and_name(self):
        ...
        ), "Should correctly parse owner and repo from the link."
    ...
    def test_have_rate_limit_enough(self):
        ...
        assert have_rate_limit(mock_client, mock_query, {}) == [
            True,
