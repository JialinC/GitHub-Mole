...
class TestClient:
    ...
    def test_client_base_path(self, github_client):
        ...
        assert (
            "api.github.com" in github_client._base_path()
        ), "Base path should include the host."
    ...
    def test_generate_headers(self, github_client, authenticator):
        ...
        assert (
            github_client._generate_headers(**additional_headers) == expected_headers
        ), "Headers should include both authenticator and additional headers."
    ...
    def test_retry_success(self, github_client, requests_mock):
        ...
        requests_mock.register_uri(
            "POST",
            github_client._base_path(),
            [{"exc": Timeout}, {"json": {"data": "success"}, "status_code": 200}],
        )

        response = github_client._retry_request("query { viewer { login }}", {})
    ...
    def test_retry_timeout(self, github_client, requests_mock):
        ...
        requests_mock.register_uri(
            "POST", github_client._base_path(), [{"exc": Timeout}, {"exc": Timeout}]
        )

        with pytest.raises(Timeout):
            github_client._retry_request("query { viewer { login }}", {})
    ...
    def test_execute_success(self, github_client, requests_mock):
        ...
        ), "Execute should return success on valid response."
    ...
    def test_execution_generator(self, github_client):
        ...
        query = MagicMock()
        query.paginator.has_next.side_effect = [
            True,
            True,
            False,
        ]  # Simulate 2 pages of results, then stop
        ...
        assert (
            results[0]["nodes"][0]["edges"] == "data1"
        ), "First result should match first mocked response"
        assert (
            results[1]["nodes"][0]["edges"] == "data2"
        ), "Second result should match second mocked response"

        # Ensure update_paginator was called correctly
        assert (
            query.paginator.update_paginator.call_count == 2
        ), "update_paginator should be called twice, once per page"
        query.paginator.update_paginator.assert_called_with(
            False, "cursor2"
        )  # Last call should reflect the end of pagination
    ...
    def test_client_execute_failed(self, github_client, requests_mock):
        ...
        assert "Query failed with code" in str(
            excinfo.value
        ), "QueryFailedException should contain the right error message."
